...
func main() {
	...
	chainCfg, err := chainconfig.ChainConfigByChainID(901)
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Failed to load devnet chain config: %v\n", err.Error())
		os.Exit(1)
	}
	runner, err := verify.NewRunner(l1RpcUrl, l1RpcKind, l1BeaconUrl, l2RpcUrl, dataDir, "", chainCfg)
	if err != nil {
