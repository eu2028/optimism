...
import (
	...
	"github.com/ethereum-optimism/optimism/op-e2e/e2eutils/mockrpc"
...
func TestScanL1(t *testing.T) {
	...
	require.NotEmpty(t, sepoliaRPCURL, "SEPOLIA_RPC_URL must be set")
	mainnetRPCURL := os.Getenv("MAINNET_RPC_URL")
	require.NotEmpty(t, mainnetRPCURL, "MAINNET_RPC_URL must be set")
	...
	t.Run("valid deployment", func(t *testing.T) {
		...
		report, err := ScanL1(ctx, retryingL1RPCClient(t, mainnetRPCURL), deploymentTx, release)
		require.NoError(t, err)
		fmt.Println(report)
	})
...
func retryingL1RPCClient(t *testing.T, url string) *rpc.Client {
	rtp := retryproxy.New(testlog.Logger(t, slog.LevelInfo), url)
	require.NoError(t, rtp.Start())
	t.Cleanup(func() {
		require.NoError(t, rtp.Stop())
	})
	rpcClient, err := rpc.Dial(rtp.Endpoint())
	require.NoError(t, err)
	return rpcClient
}
